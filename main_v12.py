"""
Exhibition Booth Configurator Bot v12 (WITH 3D CONFIGURATOR)
...
CONFIGURATOR_URL = os.getenv("CONFIGURATOR_URL", "https://your-domain.com/configurator/")
...
TEXTS = {
    ...
    "RUS": {
        ...
        "3d_configurator": "Dizajn v 3D",
        "3d_description": "Otkroj nash interaktivnyj 3D konfigurator dlya vizualizacii dizajna stenda v realnom vremeni. Ty mozhesh' vrashchat', masshtabirovat' i kustomizirovat' cveta, materialy i oborudovanie.",
        "quick_config": "Bystraya Konfig",
        "quick_config_desc": "Ili konfiguriruj bystro, ispol'zuja knopki nizhe."
    },
    "LV": {
...
def get_3d_configurator_keyboard(language, length, width, construction, materials, equipment):
    ...
    params = f"?length={length}&width={width}&construction={construction}&materials={','.join(materials)}&equipment={','.join(equipment)}"
    url = CONFIGURATOR_URL + params
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Open 3D Configurator", url=url)],
        [InlineKeyboardButton(text=get_text(language, "quick_config"), callback_data="start_config")],
        [InlineKeyboardButton(text=get_text(language, "main_menu"), callback_data="main_menu")]
    ])
    return kb, url
