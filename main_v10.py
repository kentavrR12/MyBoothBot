...
CONSTRUCTION_TYPES = {
    "standard": {"name_en": "Standard", "name_ru": "Стандарт", "name_lv": "Standarts", "price": 500.0},
    "exclusive": {"name_en": "Exclusive", "name_ru": "Эксклюзив", "name_lv": "Eksklusīvs", "price": 800.0}
}
...
def get_construction_keyboard(language):
    ...
    for key, val in CONSTRUCTION_TYPES.items():
        ...
        price = val["price"]
        buttons.append([InlineKeyboardButton(text=f"{name} (+{price:.2f}€)", callback_data=f"construction_{key}")])
...
def get_materials_keyboard(language, selected_materials):
    ...
    for key, val in MATERIALS.items():
        ...
        buttons.append([InlineKeyboardButton(text=f"{check} {name} (+{price:.2f}€)", callback_data=f"material_{key}")])
...
def get_equipment_keyboard(language, selected_equipment):
    ...
    for key, val in EQUIPMENT.items():
        ...
        buttons.append([InlineKeyboardButton(text=f"{check} {name} (+{price:.2f}€)", callback_data=f"equipment_{key}")])
...
def calculate_cost(length, width, construction_type, materials, equipment):
    ...
    try:
        ...
        # Base price (depends on construction type)
        ...
        # Materials cost
        materials_total = sum(MATERIALS.get(m, {}).get("price", 0) * area for m in materials if m in MATERIALS)
        
        # Equipment cost
        equipment_total = sum(EQUIPMENT.get(e, {}).get("price", 0) for e in equipment if e in EQUIPMENT)
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🌍 **Select your language / Выберите язык / Izvēlieties valodu:**",
...
async def choose_length(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(summary, reply_markup=get_width_keyboard(language), parse_mode="Markdown")
    await callback.answer()
...
async def choose_width(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(summary, reply_markup=get_construction_keyboard(language), parse_mode="Markdown")
    await callback.answer()
...
async def choose_construction(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(summary, reply_markup=get_materials_keyboard(language, []), parse_mode="Markdown")
    await callback.answer()
...
async def materials_done(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(summary, reply_markup=get_equipment_keyboard(language, []), parse_mode="Markdown")
    await callback.answer()
...
async def equipment_done(callback: CallbackQuery, state: FSMContext) -> None:
    """Equipment done - SHOW COST SUMMARY."""
    data = await state.get_data()
    language = data.get("language", "EN")
    ...
    logger.info(f"🔍 EQUIPMENT DONE - Data: L={length}, W={width}, Type={construction_type}, Mat={materials}, Eq={equipment}")
    ...
    await callback.message.edit_text(summary, reply_markup=get_confirmation_keyboard(language), parse_mode="Markdown")
    await callback.answer()
...
async def confirm_order(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(final_text, reply_markup=get_final_keyboard(language), parse_mode="Markdown")
    await callback.answer()
...
async def back_to_length(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    language = data.get("language", "EN")
    await state.set_state(ConfigurationStates.choosing_length)
    await callback.message.edit_text(get_text(language, "step_length"), reply_markup=get_length_keyboard(language), parse_mode="Markdown")
    await callback.answer()
...
async def back_to_materials(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(summary, reply_markup=get_materials_keyboard(language, materials), parse_mode="Markdown")
    await callback.answer()
...
async def back_to_equipment(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(summary, reply_markup=get_equipment_keyboard(language, equipment), parse_mode="Markdown")
    await callback.answer()
...
async def cancel(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.clear()
    await state.update_data(language=language)
    await callback.message.edit_text(get_text(language, "canceled"), reply_markup=get_start_keyboard(language), parse_mode="Markdown")
    await callback.answer()
...
async def main_menu(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.clear()
    await state.update_data(language=language)
    await callback.message.edit_text(get_text(language, "welcome"), reply_markup=get_start_keyboard(language), parse_mode="Markdown")
    await callback.answer()
